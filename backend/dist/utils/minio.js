...
async function ensureBucketExists() {
    const exists = await exports.minioClient.bucketExists(MINIO_BUCKET);
    if (!exists) {
        await exports.minioClient.makeBucket(MINIO_BUCKET, '');
    }
}
async function uploadFileToMinio(originalFileName, buffer, mimetype) {
    await ensureBucketExists();
    const safeFileName = sanitizeFileName(originalFileName);
    const encodedFileName = encodeURIComponent(safeFileName);
    await exports.minioClient.putObject(MINIO_BUCKET, encodedFileName, buffer, buffer.length, {
...
async function getPresignedUrl(filename) {
    ...
    const encodedFileName = encodeURIComponent(safeFileName);
    return exports.minioClient.presignedGetObject(MINIO_BUCKET, encodedFileName, 24 * 60 * 60);
}
