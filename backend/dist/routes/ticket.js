    ...
    try {
        ...
        if (req.file) {
            try {
                ...
                const { url, originalFileName } = await (0, minio_1.uploadFileToMinio)(req.file.originalname, req.file.buffer, req.file.mimetype);
        ...
        // === 이메일 알림 ===
        try {
            const toList = await (0, email_1.getUserNotificationEmails)(userId);
...
router.put('/:id', auth_1.authenticateJWT, async (req, res) => {
    try {
        ...
        if (status && prevTicket && prevTicket.status !== status) {
            try {
                const toList = await (0, email_1.getUserNotificationEmails)(prevTicket.userId);
...
router.get('/:id/messages', auth_1.authenticateJWT, async (req, res) => {
    try {
        const ticketId = Number(req.params.id);
