...
### [O] 고객사 관리 시스템
- [O] Company 모델 확장 (연락처, 계약정보, 지원형태, 설명 필드 추가)
- [O] 관리자 페이지에 고객사 관리 탭 추가 (CRUD 기능)
- [O] 사용자별 고객사 할당 기능
...
### [O] 고객사 관리 시스템 구현 (2024.07)
- Company 모델에 contact, contractInfo, supportType, description 필드 추가
- 관리자 페이지에 "고객사 관리" 탭 추가 (고객사 CRUD, 사용자 할당)
- 사용자별 기본 고객사 설정 및 티켓 생성 시 자동 적용
- 티켓 생성 페이지에 고객사 드롭다운 (기본값 표시, 직접 입력 옵션)
- 관리자가 이메일 주소별로 고객사 미리 지정 가능
- 고객사 상세 정보 관리 (연락처, 계약정보, 지원형태 등)
...
- 고객사 관리 및 사용자 할당 기능 정상 동작 확인
- 티켓 생성 시 고객사 자동 선택 및 드롭다운 기능 정상 동작 확인
